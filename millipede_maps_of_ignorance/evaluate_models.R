...
## end get spatiall subsampled test points ------------------------------------
...
for(i in 1:length(sp_to_fit)) {
  ...
  aucs <- lapply(fits, FUN = function(x, sp_name) {
    auc_iter <- lapply(x, FUN = function(y, sp_name) {
      tryCatch(as.numeric(pROC::roc(response = y$preds[, colnames(y$preds) == 
                                     gsub(" ", ".", sp_name)], 
                predictor = y$preds$pred)$auc), 
               error = function(x) NA)
    }, sp_name = sp_name)
  }, sp_name = sp_to_fit[[i]])
  ...
  aucs <- lapply(fits, FUN = function(x, sp_name, test_data) {
    auc_iter <- lapply(x, FUN = function(y, sp_name, test_data) {
      ...
      test_data <- test_data[test_data$hectad %in% y$test_sites |
                               test_data$hectad %in% y$preds$hectad, ]
      test_data <- left_join(test_data, y$preds) # join predictions to test data
      # drop checklists for which there are no predictions (sometimes checlists
      # from the same hectad are in different CV folds b/c cv block boundary is
      # within the hectad and checklists are at finer spatial resolution)
      test_data <- test_data[!is.na(test_data$pred), ] 
      tryCatch({
